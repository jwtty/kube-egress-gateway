...
func (r *PodEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, podEndpoint); err != nil {
		if apierrors.IsNotFound(err) {
			// Object not found, might be a delete event, trigger cleanUp.
			if err := r.cleanUp(ctx); err != nil {
				return ctrl.Result{}, fmt.Errorf("failed to clean up orphaned wireguard peers: %w", err)
			}
			return ctrl.Result{}, nil
	...
	if !gwConfig.ObjectMeta.DeletionTimestamp.IsZero() {
		return ctrl.Result{}, fmt.Errorf("StaticGatewayConfiguration(%s/%s) is being deleted", gwConfigKey.Namespace, gwConfigKey.Name)
	}

	// Reconcile wireguard peer
	return ctrl.Result{}, r.reconcile(ctx, gwConfig, podEndpoint, !podEndpoint.ObjectMeta.DeletionTimestamp.IsZero())
}
...
func (r *PodEndpointReconciler) reconcile(
	...
	log := log.FromContext(ctx)
	log.Info(fmt.Sprintf("Reconciling PodEndpoint %s/%s, delete: %v", podEndpoint.GetNamespace(), podEndpoint.GetName(), delete))
	...
	wgLink := getWireguardLinkName(gwConfig)
	log.Info("Configuring wireguard device", "device", wgLink, "config", wgConfig)
	if err := wgClient.ConfigureDevice(wgLink, wgConfig); err != nil {
		return fmt.Errorf("failed to add peer to wireguard device %s: %w", wgLink, err)
	}
	...
	if err := r.reconcileWireguardPeerRoutes(ctx, gwConfig, []net.IPNet{*dst}, delete); err != nil {
	...
	peerConfigs := []egressgatewayv1alpha1.PeerConfiguration{
		{
			...
			InterfaceName: wgLink,
			PublicKey:     podEndpoint.Spec.PodPublicKey,
		},
	}
	if err := r.updateGatewayNodeStatus(ctx, peerConfigs, !delete /* add */); err != nil {
		return err
	}
...
func (r *PodEndpointReconciler) cleanUp(ctx context.Context) error {
	...
	// map from gwConfig namespace/name to set of active peer public keys
	peerMap := make(map[types.NamespacedName]map[string]bool)
	for _, podEndpoint := range podEndpointList.Items {
		// skip podEndpoint that is being deleted
		if !podEndpoint.ObjectMeta.DeletionTimestamp.IsZero() {
			continue
		}
		key := types.NamespacedName{
			Namespace: strings.ToLower(podEndpoint.Namespace),
			Name:      strings.ToLower(podEndpoint.Spec.StaticGatewayConfiguration),
		}
		if _, exists := peerMap[key]; !exists {
			peerMap[key] = make(map[string]bool)
		}
		peerMap[key][podEndpoint.Spec.PodPublicKey] = true
	}
	...
	for _, gwConfig := range gwConfigList.Items {
		gwConfig := gwConfig
		// only clean up gwConfig that applies to this node and is not being deleted
		if applyToNode(&gwConfig) && isReady(&gwConfig) && gwConfig.ObjectMeta.DeletionTimestamp.IsZero() {
			activePeers := peerMap[types.NamespacedName{Namespace: strings.ToLower(gwConfig.Namespace), Name: strings.ToLower(gwConfig.Name)}]

			wgLink := getWireguardLinkName(&gwConfig)
			device, err := wgClient.Device(wgLink)
			if err != nil {
				log.Error(err, fmt.Sprintf("failed to get wireguard link %s configuration", wgLink))
				continue // do not block cleanup on error
			}

			wgConfig := wgtypes.Config{}
			var podIPsToDel []net.IPNet
			for _, peer := range device.Peers {
				if _, ok := activePeers[peer.PublicKey.String()]; !ok {
					// remove non-active peer
					...
					podIPsToDel = append(podIPsToDel, peer.AllowedIPs...)
					log.Info(fmt.Sprintf("Removing peer %s IPs %v from wireguard link %s", peer.PublicKey.String(), peer.AllowedIPs, wgLink))
				}
			}

			if len(wgConfig.Peers) > 0 {
				// first delete routes
				if err := r.reconcileWireguardPeerRoutes(ctx, &gwConfig, podIPsToDel, true /* delete */); err != nil {
					log.Error(err, fmt.Sprintf("failed to delete pod routes for link %s", wgLink))
					continue // do not block cleanup on error
				}

				if err := wgClient.ConfigureDevice(wgLink, wgConfig); err != nil {
					log.Error(err, fmt.Sprintf("failed to remove peers from wireguard device %s", wgLink))
					continue // do not block cleanup on error
				}
			}
		}
...
func (r *PodEndpointReconciler) reconcileWireguardPeerRoutes(
	ctx context.Context,
	gwConfig *egressgatewayv1alpha1.StaticGatewayConfiguration,
	podIPs []net.IPNet,
	delete bool,
	...
	log := log.FromContext(ctx)
	wgLink := getWireguardLinkName(gwConfig)
	wg, err := r.Netlink.LinkByName(wgLink)
	if err != nil {
		return fmt.Errorf("failed to retrieve wireguard link %s when reconciling routes: %w", wgLink, err)
	}

	routeTable := getPacketMarkAndTable(gwConfig)
	for _, dst := range podIPs {
		dst := dst
		route := &netlink.Route{
		...
		if delete {
			log.Info("Deleting peer route", "route", route)
			if err := r.Netlink.RouteDel(route); err != nil {
				var errno syscall.Errno
				if errors.As(err, &errno) && errno == syscall.ESRCH {
					log.Error(err, "failed to delete peer route", "route", route)
					continue
				}
				return fmt.Errorf("failed to delete peer route %s in table %d: %w", route, routeTable, err)
			}
		} else {
			log.Info("Adding peer route", "route", route)
			if err := r.Netlink.RouteReplace(route); err != nil {
				return fmt.Errorf("failed to add peer route %s in table %d: %w", route, routeTable, err)
			}
		}
	}
	return nil
