...
var _ = Describe("Daemon StaticGatewayConfiguration controller unit tests", func() {
	...
	// 			)
	// 			err := r.configureGatewayNetworking(context.TODO(), gwConfig, &pk, "10.0.0.5", "10.0.0.6")
	// 			Expect(err).To(BeNil())
	// 		})
	...
	// 				Expect(gwStatus.Spec.ReadyGatewayConfigurations[0].InterfaceName).To(Equal("ns1"))
	// 				Expect(gwStatus.Spec.ReadyPeerConfigurations[0].InterfaceName).To(Equal("ns1"))
	// 			})
	// 		})
	// 	})
	...
	// 			err := getGatewayStatus(r.Client, gwStatus)
	// 			Expect(err).To(BeNil())
	// 			Expect(gwStatus.Spec.ReadyGatewayConfigurations).To(BeEmpty())
